<#
    Скрипт для монтирвоания принтеров.
    Монтирование выполняется на основе групп "*printers access*"
    Например "Копейка printers access", или "Апполо printers access"
...
.DESCRIPTION
   Получение принтеров по коду для монтирования с принт-сервера.
